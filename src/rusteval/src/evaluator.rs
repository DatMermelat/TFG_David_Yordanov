       ...
       let mut stack: Vec<Array1<f64>> = Vec::new();
        for &t in expr {
            match t {
                ...
                "^" => {
                    let exp = stack.pop().ok_or(PyValueError::new_err("Exception during evaluation: Ran out of tokens in the stack."))?;
                    let base = stack.pop().ok_or(PyValueError::new_err("Exception during evaluation: Ran out of tokens in the stack."))?;
                    
                    if base.len() != exp.len() {
                        return Err(PyValueError::new_err("Dimension mismatch: base and exponent arrays must have the same length."));
                    }
                    
                    let result = Array1::from_iter(base.iter().zip(exp.iter()).map(|(&b, &e)| b.powf(e)));
                    stack.push(result)
                }
                "^2" => {
                    let a = stack.pop().ok_or(PyValueError::new_err("Exception during evaluation: Ran out of tokens in the stack."))?;
